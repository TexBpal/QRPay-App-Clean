...
<body>
    ...
    <script>
        var options = {
            ...
            "amount": "{{ get_amount($output['amount']->total_amount??$output['charge_calculation']['requested_amount'], null, 2) * 100 }}", // Amount is in currency subunits. Default currency is INR. Hence, 50000 refers to 50000 paise
            "currency": "{{ $output['currency']->currency_code??$output['charge_calculation']['sender_cur_code'] }}",
            ...
            "prefill": { //We recommend using the prefill parameter to auto-fill customer's contact information especially their phone number
                "name": "{{ $output['user']->fullname??$output['validated']['full_name']??"" }}", //your customer's name
                "email": "{{ $output['user']->email??$output['validated']['email']??"" }}",
